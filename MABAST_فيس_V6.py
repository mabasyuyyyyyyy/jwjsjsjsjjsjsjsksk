...
def menu(my_name,my_id):	
	...
	banner()
	print('')
	print('\033[1;33m» 𝐌𝐘 𝐂𝐇𝐀𝐍𝐍𝐄𝐋 ⇒ @r_e_555        ')
	print('') 
	print('') 
	os.system('clear')
	print('''
───────────────────────────────────────────────────\n\x1b[1;32m(\x1b[1;35m+\x1b[1;32m) \x1b[1;33m Telegram\x1b[1;35m | \x1b[1;32m @r_e_555 \x1b[1;35m (\x1b[1;32m+\x1b[1;35m) \x1b[1;33m instgram : 3fw_0  \x1b[1;32m\n\x1b[1;───────────────────────────────────────────────────   ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀ ''')
...
def passwrd():
    ...
    مبستi('')
    cetak(nel('t[cyan]✓[green] Crack Selesai Ngab, Jangan Lupa Bersyukur[cyan] ✓[white] '))
    ...
    if woi in ('y', 'Y'):
        ...
        مبستi(f'''t{x}[=]{k} Been completed {x} <> ''')
        time.sleep(2)
        exit()
...
def crack(idf, pwv):
	...
	rc = random.choice
	sys.stdout.write(f"\r{bo}[MABAST ] - {loop}/{len(id)} - [OK:{ok}] - [CP:{cp}] {'{:.0%}'.format(loop/float(len(id)))}    "),
	sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					مبستi('n')
					statuscp = f''' ▄︻デ══━一 «𝗖𝗣  سڪيوࢪ  مع الاسف ❌💔» ⏎
⫘⫘⫘⫘⫘⫘⫘⫘⫘⫘⫘⫘
...
n● مبست n - 𝐂𝐎𝐎𝐊𝐈𝐄𝐒 ッ n:{kuki}
nBy مبست n:『@r_e_555 』⏎'nnttttt
					'''
					...
					infoakun += f''' ▄︻デ══━一 «𝗢𝗞 مبست جابلك حساب شغال 🤩⚜️
 » ⏎
...
مبست :لاتنسه صور الصيد『@r_e_555 』⏎'''
					...
					مبستi('n')
					statusok = f''' ▄︻デ══━一 «𝗢𝗞 مبست جابلك حساب شغال 🤩⚜️
 » ⏎
